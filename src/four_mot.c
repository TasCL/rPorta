 ...
 */
{
    ...
    for (itr = equa_in; itr != 0; itr--)
    {
        ...
        if (indx[pivcol] >= 0)
        {
            ...
            /* Store it in ar4 */
            nel_ar4 += eqrl;
            ar4 = (RAT *) RATallo(RP ar4,U nel_ar4-eqrl,U nel_ar4);
            sptr = ar4+(*equa)*eqrl;
...
//            fprintf(prt,"\nstoring finite inequalities in file %s\n\n",fname);
    ...
    ar2 =  (RAT *) RATallo(RP ar2,nel_ar2,U 2*nieq*rowl);
    sptr = ar2+nieq*rowl;
    ...
    ar2 =  (RAT *) RATallo(RP ar2,U 2*nieq*rowl,U nieq*rowl);
    nel_ar2 = nieq*rowl;
    ...
    ar3 = (RAT *) RATallo(RP ar3,0,U nel_ar3);
    ar3bd = ar3+nel_ar3-1;
...
// MG: This cast: (long) (porta_list[i]->ptr) generates a compiler error, since porta_list[]->ptr is of type RAT*.
//     The first element of RAT* is a long... is that what's being accessed?!
#pragma GCC diagnostic ignored "-Wpointer-to-int-cast"
              if (!(long) (porta_list[i]->ptr)) nn++;                   // MG: THROWS CAST WARNING
                else if ((long) (porta_list[i]->ptr) < neg)             // MG: THROWS CAST WARNING
                    neg = (long) (porta_list[i]->ptr)/(sizeof(RAT));    // MG: THROWS CAST WARNING
#pragma GCC diagnostic warning "-Wpointer-to-int-cast"
            }
            if ((nf_dstf = nn) > finie)
            {
            	//TODO: Was wird hier geschrieben?
                if (poi_file)
                {
                    fie = fopen (fname,"a");
                    for (i = ineq-1; nn > finie; i--)
#pragma GCC diagnostic ignored "-Wpointer-to-int-cast"
                        if (!(long)(porta_list[i]->ptr))                // MG: THROWS CASE WARNING
#pragma GCC diagnostic warning "-Wpointer-to-int-cast"
                        {
...
//        fprintf(prt,"testing strong validity ");
    ...
    for (i = 0; i < points; i++)
    {
    	#ifdef _DEBUG
    	printf("%i ",i);
    	printf("indx == 0: %i", indx == 0);
    	printf("dim-equa: %i ",dim-equa);
    	#endif
    	for (j = 0; j < dim-equa; j++){
    		#ifdef _DEBUG
            printf("index %i: %i", j,indx[j]);
            #endif
    ...
    *rowl_inar = dim-equa+1;
    ...
    nel_ar5 = 3*(sysrow+1);
    ar5 = (RAT *) RATallo (RP ar5,0,nel_ar5);
    convmid = ar5;
    mid = ar5+sysrow+1;
    x = mid+sysrow+1;
