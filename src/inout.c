...
//---------------------------------------------------------------HilfsMethode fuer FehlerBehandlung in scan_line{
...
/*    char * pufferLine ;
    ...
    {
    	//-------------------------------------------------------------------error{
//    	pufferLine = strtok (in_line,"/ ");

    	val.num = (int)strtol(in_line,&ptr,10);
/*    	sprintf(pufferNum, "%i", val.num);

    	#ifdef _DEBUG
    	printf("\npufferLine %s ",pufferLine);
    	printf("\npufferNum  %s\n",pufferNum);
    	#endif

    	if( !equals(pufferLine,pufferNum)){
       		msg("\n%s invalid format, maxValue of Numerator must be <= %i\n","",val.num);
    	}*/
...
/*    			sprintf(pufferDen, "%i", val.den.i);
    			...
    			if( !equals(pufferLine,pufferDen)){
       				msg("\n%s invalid format, maxValue of denominator must be <= %i","",val.den.i);
    			}*/
                ...
                if (ptr == in_line || val.den.i <= 0)
                    msg("%s, line %i : invalid denominator",fname,line);
                in_line = ptr;
            }
            else
            {
 ...
 */
{
    int i,j,ieqs,nonempty,arrows=0,arrowl=0,line;
    cone = 0;conv = 0;
    int *hip;
    char *in,*end = "END",
    equalities[22],
...
/*  // MG: The wfopen function is UNUSED throughout
#include <sys/stat.h>
...
/*
	fmel: write_ieq_file(*argv,outfp,equa,ineq,dim+1,
							0,ineq,0,ierl,indx);
	traf: write_ieq_file(*argv,outfp,equa,ineq,dim+1,
							0,ineq,0,dim+1-equa,indx);
*/


void constructIeqSexp(int dim,int n_ineqeq, RAT * ar1,int nel_ar1, RAT * ar6, int nel_ar6, int * elim_ord){
	// elim_ord[0,1,...,dim]
	// n_ineqeq stores the number of inequalities plus equalities stored in ar1
	// ar1 array of inequalities and equalities (row_major?) row length = dim+2
	// where ar1[dim+1] = 0 stands for an equation   (?)
	// and   ar1[dim+1] = 1 stands for an inequality (?)
	int j,i;
	//printf("Debug: ConstructIeqSexp START!\n");

	SEXP TYPE, VALID_NUM, VALID_DEN, INEQUALITIES_NUM,INEQUALITIES_DEN,EQUALITIES_NUM,EQUALITIES_DEN,STRONG_VALIDITY;

	PROTECT(IEQPOI=allocVector(VECSXP,8));
	PROTECT(TYPE = allocVector(INTSXP,1));
	INTEGER(TYPE)[0]=0;
	PROTECT(VALID_NUM = allocMatrix(INTSXP,1,dim));
	PROTECT(VALID_DEN = allocMatrix(INTSXP,1,dim));

	//printf("Debug: Alloziiere Speichplatz fuer: \n");
	//printf("%i Inequlities \n", ineq);
	//printf("%i Equalities \n\n", equa);

	PROTECT(EQUALITIES_NUM = allocMatrix(INTSXP,equa,dim+1));
	PROTECT(EQUALITIES_DEN = allocMatrix(INTSXP,equa,dim+1));
	PROTECT(INEQUALITIES_NUM = allocMatrix(INTSXP,ineq,dim+1));
	PROTECT(INEQUALITIES_DEN = allocMatrix(INTSXP,ineq,dim+1));
	PROTECT(STRONG_VALIDITY = allocMatrix(LGLSXP,ineq,points));


	RAT * akt_valid = ar6;

	//printf("Debug: Valid einlesen\n");


	for (i = 0; i < dim; i++){
		//printf("Debug: %ite-Valid_komponente: %i/%i \n",i,(akt_valid+i)->num,(akt_valid+i)->den.i);
		INTEGER(VALID_NUM)[i] = (akt_valid+i)->num;
		INTEGER(VALID_DEN)[i] = (akt_valid+i)->den.i;
	}

	//printf("Debug: Equalities/Inequalities einlesen\n");

	RAT * akt_ieq = ar1;
	int ie = 0;
	int eq = 0;
	for (i = 0; i < n_ineqeq; i++){
		if ((akt_ieq + (dim+1))->num==0){
			//printf("Debug: %ite-Equality:",eq);
			for (j = 0; j < dim+1; j++){
				//printf(" %i/%i " , (akt_ieq+j)->num,(akt_ieq+j)->den.i);
				INTEGER(EQUALITIES_NUM)[j*equa+eq] = (akt_ieq+j)->num;
		    	INTEGER(EQUALITIES_DEN)[j*equa+eq] = (akt_ieq+j)->den.i;
			}
			//printf("\n");
			eq++;
		}
		if ((akt_ieq+(dim+1))->num == 1){
			//printf("Debug: %ite-Inequality:",ie);
			for (j = 0; j < dim+1; j++){
				//printf(" %i/%i " , (akt_ieq+j)->num,(akt_ieq+j)->den.i);
				INTEGER(INEQUALITIES_NUM)[j*ineq+ie] = (akt_ieq+j)->num;
				INTEGER(INEQUALITIES_DEN)[j*ineq+ie] = (akt_ieq+j)->den.i;
			}
			//printf("\n");
			ie++;
		}
	...
	//printf("Debug: Antwort zusammenbauen\n");


	SET_VECTOR_ELT(IEQPOI,0,TYPE);
	SET_VECTOR_ELT(IEQPOI,1,VALID_NUM);
	SET_VECTOR_ELT(IEQPOI,2,VALID_DEN);
	SET_VECTOR_ELT(IEQPOI,3,EQUALITIES_NUM);
	SET_VECTOR_ELT(IEQPOI,4,EQUALITIES_DEN);
	SET_VECTOR_ELT(IEQPOI,5,INEQUALITIES_NUM);
	SET_VECTOR_ELT(IEQPOI,6,INEQUALITIES_DEN);

	//printf("Debug: Unprotect\n");

	UNPROTECT(8);
}


void constructPoiSexp(int dim,int n_points, RAT * ar1,int nel_ar1){
	// insert Construction Code here
	int i,j;
	SEXP TYPE, CONV_SECTION_NUM, CONE_SECTION_NUM,CONV_SECTION_DEN, CONE_SECTION_DEN,STRONG_VALIDITY;

	PROTECT(IEQPOI=allocVector(VECSXP,6));

#ifdef _DEBUG   // MG: Wrapped with #ifdef _DEBUG
	printf("Debug: Speicher alloziieren: \n");
	printf("%i Cone Points \n", cone);
	printf("%i Conv Points \n\n",conv);
#endif
	...
	PROTECT(STRONG_VALIDITY = allocMatrix(LGLSXP,ineq,points-1));

	RAT * akt_point = ar1;
	int ce = 0;
	int cv = 0;
	for (i = 0; i < n_points; i++){
		if ((akt_point + (dim))->num==0){ // CONE POINT
			//printf("Debug: %ite-Equality:",eq);
			for (j = 0; j < dim; j++){
				//printf(" %i/%i " , (akt_ieq+j)->num,(akt_ieq+j)->den.i);
				INTEGER(CONE_SECTION_NUM)[j*cone+ce] = (akt_point+j)->num;
		    	INTEGER(CONE_SECTION_DEN)[j*cone+ce] = (akt_point+j)->den.i;
			}
			//printf("\n");
			ce++;
		}
		if ((akt_point+(dim))->num == 1){ // CONV POINT
			//printf("Debug: %ite-Inequality:",ie);
			for (j = 0; j < dim; j++){
				//printf(" %i/%i " , (akt_ieq+j)->num,(akt_ieq+j)->den.i);
				INTEGER(CONV_SECTION_NUM)[j*conv+cv] = (akt_point+j)->num;
				INTEGER(CONV_SECTION_DEN)[j*conv+cv] = (akt_point+j)->den.i;
			}
			//printf("\n");
			cv++;
		}
	...
	SET_VECTOR_ELT(IEQPOI,0,TYPE);
	if (cone > 0) SET_VECTOR_ELT(IEQPOI,1,CONE_SECTION_NUM);
	if (cone > 0) SET_VECTOR_ELT(IEQPOI,2,CONE_SECTION_DEN);
	if (conv > 0) SET_VECTOR_ELT(IEQPOI,3,CONV_SECTION_NUM);
	if (conv > 0) SET_VECTOR_ELT(IEQPOI,4,CONV_SECTION_DEN);
	UNPROTECT(6);
                     ...
                     int *eqindx, int ineq, int fie, int ierl, int *ieindx )
{
	// equa ^= equation
	// feq  ^= first equation
	// eqrl ^= equation row length
	// eqindx ^= equation indices
	// ineq ^= inequality
	// fie ^= first inequality
	// ierl = iniequality row length
	// ieindx ^= inequality indices

	#ifdef _DEBUG
		printf("Write IEQ START!\n");
	#endif
    char filename[100];
    int i; //, start;  MG: UNUSED
    ...
    if (!fp)
    {
        ...
        if (is_set(Traf) )
        {
            ...
            for (i = 0; i< dim; i++)
            {
            	INTEGER(VALID_NUM)[i] = ar6[i].num;
                INTEGER(VALID_DEN)[i] = ar6[i].den.i;
            }
            //fprintf(fp, "\n\n");
        }
    }
...
//void writesys( FILE *fp, int frow, int lrow, int rowl, int format, int *indx, char eqie, int *start ){ \\  -{

	    int i,j;

	    // TODO @Adalet: Wird das hier gebraucht ? (by Sebastian)
	    if (!MP_realised)
	    {
	        max = (RAT *) RATallo(RP 0,0,U eqrl);
	        for (j = 0; j < eqrl; j++)
	            max[j].num =  max[j].den.i = 0;

	        for (i = feq; i < feq+equa; i++)
	            max_vals( max, porta_list[i]->sys, eqrl );
	        width_line(max,eqrl,0);
	    }
    ...
    //
	   ...
	   if (eqindx){
	    	for (i = 0; i < dim+1; i++){
	       		for (j = 0; j < equa; j++){
	    			INTEGER(EQUALITIES_NUM)[i*equa+j] = 0;
					INTEGER(EQUALITIES_DEN)[i*equa+j] = 1;    	 // TODO: habe 0 in 1 geaendert auf verdacht
	       		}
	    	}
	    	for (i = 0; i < eqrl; i++){
       			int ind=eqindx[i];
       			if (i==eqrl-1) ind=dim;
	       		for (j = 0; j < equa; j++){
	    			INTEGER(EQUALITIES_NUM)[ind*equa+j] = (porta_list[feq+j]->sys+i)->num;
					INTEGER(EQUALITIES_DEN)[ind*equa+j] = (porta_list[feq+j]->sys+i)->den.i;
	    		}
	   	    }
	    } else {
	    	for (i = 0; i < eqrl; i++){
	       		for (j = 0; j < equa; j++){
	        		INTEGER(EQUALITIES_NUM)[i*equa+j] = (porta_list[feq+j]->sys+i)->num;
	    			INTEGER(EQUALITIES_DEN)[i*equa+j] = (porta_list[feq+j]->sys+i)->den.i;
	    	 	}
	       	}
	    }
    ...
    for (i = feq; i < feq+equa; i++)
    {
    	fprintf(fp,"(%3d) ",start);
        (start)++;
        (* writeline)(fp,eqrl,porta_list[i]->sys,0,max,'=',eqindx);
 //void I_RAT_writeline( FILE *fp, int col, RAT *ptr, int format, RAT *max, char ie_eq, int *indx );
        if (option & Statistic_of_coefficients)
            writestatline(fp,(int *)porta_list[i]->ptr);
        fprintf(fp,"\n");
    }
    if (!MP_realised) RATallo(max,eqrl,0);  //------------------------------------------                      -}
    ...
    */
    ...
    //''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''''
	    ...
	    if (ieindx){
	    	for (i = 0; i < dim+1; i++){
	       		for (j = 0; j < ineq; j++){
	    			INTEGER(INEQUALITIES_NUM)[i*ineq+j] = 0;
	   				INTEGER(INEQUALITIES_DEN)[i*ineq+j] = 1;
	       		}
	    	}
	    	for (i = 0; i < ierl; i++){
       			int ind=ieindx[i];
       			if (i==ierl-1) ind=dim;
	       		for (j = 0; j < ineq; j++){
	    			INTEGER(INEQUALITIES_NUM)[ind*ineq+j] = (porta_list[fie+j]->sys+i)->num;
	   				INTEGER(INEQUALITIES_DEN)[ind*ineq+j] = (porta_list[fie+j]->sys+i)->den.i;
	    		}
	   	    }
	    } else {
	    // writesys(fp,fie,fie+ineq,ierl,0,ieindx,'<',&start);
	    	for (i = 0; i < ierl; i++){
	    		for (j = 0; j < ineq; j++){
					...
					INTEGER(INEQUALITIES_DEN)[i*ineq+j] = (porta_list[fie+j]->sys+i)->den.i;
		 		}
	     	}
	    }
    }
    ...
    if (is_set(Validity_table_out)) {
     //   writepoionie(fp,fie,fie+ineq,points,0);
       ...
       for (j = 0; j < ineq; j++){
       		...
       		int b,column;
       		unsigned m;
		    for (b = 0, column=0 ; b < (points)/32+1 ; b++)
		    {
		    	#ifdef _DEBUG
		    	printf("j,b,fie: (%i,%i,%i)\n",j,b,fie);
		    	#endif
		        m = (porta_list[j+fie]->mark)[b];
		        for (i = 0; i < ((b < (points)/32) ? 32 : (points)%32); i++,column++)
		        {
		        #ifdef _DEBUG
		        	printf("(%i,%i)->(%i)=%i\n",column,j,column*ineq+j,m&1);
				#endif
	...
	SET_VECTOR_ELT(IEQPOI,0,TYPE);
	if (is_set(Traf)) SET_VECTOR_ELT(IEQPOI,1,VALID_NUM);
	if (is_set(Traf)) SET_VECTOR_ELT(IEQPOI,2,VALID_DEN);
	SET_VECTOR_ELT(IEQPOI,3,EQUALITIES_NUM);
	SET_VECTOR_ELT(IEQPOI,4,EQUALITIES_DEN);
	SET_VECTOR_ELT(IEQPOI,5,INEQUALITIES_NUM);
	SET_VECTOR_ELT(IEQPOI,6,INEQUALITIES_DEN);
	if (is_set(Validity_table_out)) SET_VECTOR_ELT(IEQPOI,7,STRONG_VALIDITY);
...
//    	 	    	INTEGER(CONE_SECTION_DEN)[i*cone+j] = (porta_list[flr+j]->sys+i)->den.i;
   ...
   SET_VECTOR_ELT(IEQPOI,0,TYPE);
   if (cone > 0 || lr > 0) SET_VECTOR_ELT(IEQPOI,1,CONE_SECTION_NUM);
   if (cone > 0 || lr > 0) SET_VECTOR_ELT(IEQPOI,2,CONE_SECTION_DEN);
   if (conv > 0) SET_VECTOR_ELT(IEQPOI,3,CONV_SECTION_NUM);
   if (conv > 0) SET_VECTOR_ELT(IEQPOI,4,CONV_SECTION_DEN);
...
void writepoionie( FILE *fp, int fineq, int lineq, int points, int poi_ieq )
{
  ...
  free(sumie);
#else // WIN32
  cfree(sumie); // [MG] Was commented out, would cause memory leak?
#endif
}
...
void oldwrite_poi_file( char *fname, FILE *fp, int dim, int lr, int flr,
                     int cone, int fce, int conv, int fcv )
{
    char filename[100];
    int i,j,start;
